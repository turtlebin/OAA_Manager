 ...
 */
public class DataSync2 implements Runnable {//这个为段3的定时同步
	...
	public DataSync2(Integer paragraphID) {
		this.paragraphID = paragraphID;
		this.force = false;
		this.success = false;;
	}

	public DataSync2(Integer paragraphID, boolean force) {
		this.paragraphID = paragraphID;
		this.force = force;
	}
		...
		try {
			while (rs.next()) {
				...
				if (this.force||syncDirectionType.equals(DriverConstant.SyncDirectionType_0)) {
						// 数据源->数格引擎
					try {
						success=new SyncHelper2().joinSync(paragraphName);
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}//进行同步
