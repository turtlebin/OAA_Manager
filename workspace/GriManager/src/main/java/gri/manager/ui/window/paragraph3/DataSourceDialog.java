...
public class DataSourceDialog extends Dialog {
	...
	public Map<String,Map<String,Column>> tableMap=new HashMap<String,Map<String,Column>>();//<数据源字符串,<目标列字符串,数据源对应Column>
	public Map<String,Column> columnMap=new HashMap<String,Column>();

//	private List<Column> columns=new ArrayList<Column>();

	
	public DataSourceDialog(int windowType,Shell parent, int style,List<Column> columns) {
		super(parent, style);
	...
	private void createContents() {
		...
		button_show_firm.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				tableSource=type+"###"+host+"###"+dbName+"###"+user+"###"+password+"###"+port+"###"+tableName;
    	...
    	table = new Table(composite_group, SWT.SINGLE);  
...
//		text_db_sql.setBounds(83, 18, 308, 146);
		...
		button_ok.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				case 0:
					...
					int mapCount = MapUtil.countMap(columnMap);
					if (mapCount == total2) {
						canClose = true;
					} else {
						showMessage("请填写完整且正确的数据列对应信息", shell);
						canClose = false;
						return;
					}
				}
	...
	private List<Column> getColumnList(DBHelper2 db) {
		...
		try {
			...
			case "SQL SERVER":selectSql="select top 2 * from "+db.getTableName();break;
			default:selectSql="";
			}
//			if (db.getType().equalsIgnoreCase("MYSQL")) {
//				selectSql = "select * from `" + db.getTableName() + "` limit 1";
//			} else {
//				selectSql = "select * from " + db.getTableName()+" where rownum<5";
//			}
					...
					Display.getDefault().asyncExec(new Runnable() {
						public void run() {
							...
							DataSourceColumnDialog dialog = new DataSourceColumnDialog(Constant.WindowType_Add, shell,
									SWT.Close | SWT.APPLICATION_MODAL, tableSource,columns);
							dialog.open();
