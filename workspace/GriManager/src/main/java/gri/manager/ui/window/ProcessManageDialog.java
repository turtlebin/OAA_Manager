...
public class ProcessManageDialog extends Dialog {
	 ...
	 * @param style
	 */
	public ProcessManageDialog(ProcessManager manager, Paragraph paragraph, String account, Shell parent, int style) {
		super(parent, style);
	...
	private void createContents() {
				...
				if (e.button == 3) {
					...
					item_refresh.addListener(SWT.Selection, new Listener() {
						...
						public void handleEvent(Event event) {
							processs = manager.listProcess(paragraph.getId());							
							tableViewer.refresh();
						}
					});
		...
		this.table.setBounds(10, 70, 227, 129);
		...
		toolItem_view.setImage(new Image(toolBar.getDisplay(), this.getClass().getResourceAsStream("/icons/query24.png")));
		...
		String[] heards = new String[] { "段", "处理器", "容器" , "处理配置"};
		for (int i = 0; i < heards.length; i++) {
			TableViewerColumn column = new TableViewerColumn(tableViewer, SWT.LEFT);
			column.getColumn().setText(heards[i]);
			column.getColumn().setWidth(60);
		}
		tableViewer.setLabelProvider(new ProcessTableLabelProvider());
		tableViewer.setContentProvider(new ProcessTableContentProvider());
	...
	private void delete() {
		...
		Process process = (Process) item1.getData();
		MessageBox messageBox = new MessageBox(shell, SWT.APPLICATION_MODAL | SWT.YES | SWT.NO);
		messageBox.setText("提示");
		messageBox.setMessage("确定删除？");
		if (messageBox.open() == SWT.YES) {
			boolean succeed = manager.delProcess(process.getId());
			if (succeed) {
				RunTimeData.userView.remove(process);
	...
	private void add() {
		...
		process.setParagraphId(paragraph.getId());
		process.setParagraphName(paragraph.getName());
		new ProcessDialog(manager, Constant.WindowType_Add, account, process, shell, SWT.CLOSE | SWT.APPLICATION_MODAL).open();
		processs = manager.listProcess(paragraph.getId());
		tableViewer.refresh();
	}
	...
	private void edit() {
		...
		TableItem item1 = (TableItem) table.getSelection()[0];
		Process process = (Process) item1.getData();
		new ProcessDialog(manager, Constant.WindowType_Edit, account, process, shell, SWT.CLOSE | SWT.APPLICATION_MODAL).open();
		processs = manager.listProcess(paragraph.getId());
		tableViewer.refresh();
	}
	...
	private void view() {
		...
		TableItem item1 = (TableItem) table.getSelection()[0];
		Process process = (Process) item1.getData();
		new ViewContentDialog(manager, process, shell, SWT.CLOSE | SWT.APPLICATION_MODAL).open();
	}
	...
	public class ProcessTableContentProvider implements IStructuredContentProvider {
		...
		public Object[] getElements(Object element) {
			if (element.equals("root"))
				return processs.toArray(new Process[0]);
			else
				return null;
		}
	}
	
	public class ProcessTableLabelProvider extends LabelProvider implements ITableLabelProvider {
		...
		public String getColumnText(Object obj, int index) {
			if (obj instanceof Process) {
				...
				if (index == 0)
					return process.getParagraphName();
				else if (index == 1)
					return process.getProcessorName();
				else if (index == 2)
					return process.getContainerName();
				else if (index == 3)
